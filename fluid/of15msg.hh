...
#include "of15/of15action.hh"
#include "of15/of15common.hh"
#include "of15/of15meter.hh"
...
/**
 Base class for OpenFlow 1.5 Role messages.
 */
...
public:
    ...
    uint64_t generation_id() {
        return this->generation_id_;
    }
    void role(uint32_t role) {
        this->role_ = role;
    }
...
/**
 Base class for OpenFlow 1.5 Async Config messages.
 */
...
public:
    ...
    uint32_t slave_packet_in_mask() {
        return this->packet_in_mask_[1];
    }
    uint32_t slave_port_status_mask() {
        return this->port_status_mask_[1];
    }
...
/**
 Classes for creating and parsing OpenFlow 1.5 messages.
 */

/**
 OpenFlow 1.5 OFPT_HELLO message
 */
...
public:
    ...
    uint8_t* pack();
    of_error unpack(uint8_t* buffer);
    std::list<HelloElemVersionBitmap> elements() {
...
/**
 OpenFlow 1.5 OFPT_ERROR message.
 */
...
/**
 OpenFlow 1.5 OFPT_ECHO_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_ECHO_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_EXPERIMENTER message.
...
//    }
//    uint32_t exp_type() {
//        return this->exp_type_;
//    }
//};

/**
 OpenFlow 1.5 OFPT_FEATURES_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_FEATURES_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_GET_CONFIG_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_GET_CONFIG_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_SET_CONFIG_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_PACKET_OUT message.
 */
...
public:
    PacketOut();
    PacketOut(uint32_t xid, uint32_t buffer_id);
...
/**
 OpenFlow 1.5 OFPT_PACKET_IN message.
 */
...
/**
 OpenFlow 1.5 OFPT_FLOW_MOD message.
 */
...
private:
    ...
    uint32_t out_port_;
    uint32_t out_group_;
    uint16_t importance_;
    of15::Match match_;
    InstructionSet instructions_;
public:
    FlowMod();
    FlowMod(uint32_t xid, uint64_t cookie, uint64_t cookie_mask,
    ...
    of15::InstructionSet instructions() {
    ...
    void out_group(uint32_t out_group) {
        this->out_group_ = out_group;
    }
    void importance(uint16_t importance) {
...
/**
 OpenFlow 1.5 OFPT_FLOW_REMOVED message.
 */
class FlowRemoved: public OFMsg {
private:
    uint8_t table_id_;
    uint8_t reason_;
    uint16_t priority_;
...
public:
    FlowRemoved();
    FlowRemoved(uint32_t xid, uint8_t table_id, uint8_t reason, uint16_t priority,
        uint16_t idle_timeout, uint16_t hard_timeout, uint64_t cookie);
    FlowRemoved(uint32_t xid, uint8_t table_id, uint8_t reason, uint16_t priority,
        uint16_t idle_timeout, uint16_t hard_timeout, uint64_t cookie, of15::Match,
        of15::Stats);
    ~FlowRemoved() {
    }
    virtual uint16_t length();
    ...
    void stats(of15::Stats stats) {
        this->stats_ = stats;
    }
};

/**
 OpenFlow 1.5 OFPT_PORT_STATUS message.
 */
...
public:
    PortStatus();
    PortStatus(uint32_t xid, uint8_t reason, of15::Port desc);
    ~PortStatus() {
    }
    ...
    void desc(of15::Port desc) {
        this->desc_ = desc;
    }
};

/**
 OpenFlow 1.5 OFPT_PORT_MOD message.
 */
...
/**
 OpenFlow 1.5 OFPT_GROUP_MOD message.
 */
...
/**
 OpenFlow 1.5 OFPT_TABLE_MOD message.
 */
...
/**
 OpenFlow 1.5 OFPT_MULTIPART_REQUEST message header. Multipart request
...
/**
 OpenFlow 1.5 OFPT_MULTIPART_REPLY message header. Multipart reply
 messages should inherit from this class.
 */
...
/**
 OpenFlow 1.5 OFPMP_DESC multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_DESC multipart reply.
 */
...
public:
    ...
    MultipartReplyDesc(uint32_t xid, uint16_t flags, std::string mfr_desc,
        std::string hw_desc, std::string sw_desc, std::string serial_num,
        std::string dp_desc);
    ~MultipartReplyDesc() {
    }
    ...
    void set_desc(SwitchDesc desc) {
        this->desc_ = desc;
    }
};

/**
 OpenFlow 1.5 OFPMP_FLOW multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_FLOW multipart reply.
 */
class MultipartReplyFlow: public MultipartReply {
private:
    std::vector<of15::FlowStats> flow_stats_;
public:
    MultipartReplyFlow();
    ...
    void flow_stats(std::vector<of15::FlowStats> flow_stats);
    void add_flow_stats(of15::FlowStats);
};

/**
 OpenFlow 1.5 OFPMP_AGGREGATE multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_AGGREGATE multipart reply.
 */
...
/**
 OpenFlow 1.5 OFPMP_TABLE multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_TABLE multipart reply.
 */
class MultipartReplyTable: public MultipartReply {
private:
    std::vector<of15::TableStats> table_stats_;
public:
    MultipartReplyTable();
    ...
    void table_stats(std::vector<of15::TableStats> table_stats);
    void add_table_stat(of15::TableStats stat);
};

/**
 OpenFlow 1.5 OFPMP_PORT_STATS multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_PORT_STATS multipart reply.
 */
class MultipartReplyPortStats: public MultipartReply {
private:
    std::vector<of15::PortStats> port_stats_;
public:
    ...
    void port_stats(std::vector<of15::PortStats> port_stats);
    void add_port_stat(of15::PortStats stat);
};

/**
 OpenFlow 1.5 OFPMP_QUEUE multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_QUEUE multipart reply.
 */
class MultipartReplyQueue: public MultipartReply {
private:
    std::vector<of15::QueueStats> queue_stats_;
public:
    MultipartReplyQueue();
    ...
    void queue_stats(std::vector<of15::QueueStats> queue_stats_);
    void add_queue_stat(of15::QueueStats stat);
};

/**
 OpenFlow 1.5 OFPMP_GROUP multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_GROUP multipart reply.
 */
class MultipartReplyGroup: public MultipartReply {
private:
    std::vector<of15::GroupStats> group_stats_;
public:
    MultipartReplyGroup();
    ...
    uint8_t* pack();
    of_error unpack(uint8_t *buffer);
    std::vector<of15::GroupStats> queue_stats() {
        return this->group_stats_;
    }
    void group_stats(std::vector<of15::GroupStats> group_stats);
    void add_group_stats(of15::GroupStats stat);
    size_t group_stats_len();
};

/**
 OpenFlow 1.5 OFPMP_GROUP_DESC multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_GROUP_DESC multipart reply.
 */
class MultipartReplyGroupDesc: public MultipartReply {
private:
    std::vector<of15::GroupDesc> group_desc_;
public:
    ...
    uint8_t* pack();
    of_error unpack(uint8_t *buffer);
    std::vector<of15::GroupDesc> queue_stats() {
        return this->group_desc_;
    }
    void group_desc(std::vector<of15::GroupDesc> group_desc);
    void add_group_desc(of15::GroupDesc stat);
    size_t desc_len();
};

/**
 OpenFlow 1.5 OFPMP_GROUP_FEATURES multipart request.
 */
...
public:
    ...
    void features(of15::GroupFeatures features) {
        this->features_ = features;
    }
};

/**
 OpenFlow 1.5 OFPMP_METER multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_METER multipart reply.
 */
class MultipartReplyMeter: public MultipartReply {
private:
    std::vector<of15::MeterStats> meter_stats_;
public:
    MultipartReplyMeter();
    ...
    void meter_stats(std::vector<of15::MeterStats> meter_stats);
    void add_meter_stats(MeterStats stats);
    size_t meter_stats_len();
};

/**
 OpenFlow 1.5 OFPMP_METER_CONFIG multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_METER_CONFIG multipart reply.
 */
...
/**
 OpenFlow 1.5 OFPMP_METER_FEATURES multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_METER_FEATURES multipart reply.
 */
...
/**
 OpenFlow 1.5 OFPMP_TABLE_FEATURES multipart request.
 */
...
/**
 OpenFlow 1.5 OFPMP_TABLE_FEATURES multipart reply.
 */
...
/**
 OpenFlow 1.5 OFPMP_PORT_DESC multipart request.
 */
...
public:
    ...
    void ports(std::vector<of15::Port> ports);
    void add_port(of15::Port);
};

/**
 OpenFlow 1.5 OFPMP_EXPERIMENTER multipart request.
...
/**
 OpenFlow 1.5 OFPMP_EXPERIMENTER multipart reply.
...
/**
 OpenFlow 1.5 OFPT_BARRIER_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_BARRIER_REPLY message*/
...
//Aman
//TODO: Change to multpart request
///**
// OpenFlow 1.5 OFPT_QUEUE_GET_CONFIG_REQUEST message.
// */
...
//
///**
// OpenFlow 1.5 OFPT_QUEUE_GET_CONFIG_REPLY message.
// */
//class QueueGetConfigReply: public OFMsg {
//private:
//    uint32_t port_;
//    std::list<of15::PacketQueue> queues_;
//public:
//    QueueGetConfigReply();
...
//        this->port_ = port;
//    }
//    void queues(std::list<of15::PacketQueue> queues);
//    void add_queue(PacketQueue queue);
//    size_t queues_len();
//};

/**
 OpenFlow 1.5 OFPT_ROLE_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_ROLE_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_GET_ASYNC_REQUEST message.
 */
...
/**
 OpenFlow 1.5 OFPT_GET_ASYNC_REPLY message.
 */
...
/**
 OpenFlow 1.5 OFPT_SET_ASYNC message.
 */
