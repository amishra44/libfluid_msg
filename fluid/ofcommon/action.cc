...
bool Action::equals(const Action &other) {
    return ((*this == other));
}
...
ActionList::~ActionList() {
    this->action_list_.remove_if(Action::delete_all);
}
...
size_t ActionList::pack(uint8_t *buffer) {
    uint8_t *p = buffer;
    for (std::list<Action*>::iterator it = this->action_list_.begin(), end =
        this->action_list_.end(); it != end; ++it) {
        (*it)->pack(p);
        p += (*it)->length();
    }
    return 0;
...
size_t ActionSet::pack(uint8_t *buffer) {
    uint8_t *p = buffer;
    for (std::set<Action*>::iterator it = this->action_set_.begin(), end =
        this->action_set_.end(); it != end; ++it) {
        (*it)->pack(p);
        p += (*it)->length();
    }
    return 0;
