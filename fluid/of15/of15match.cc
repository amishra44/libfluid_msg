...
size_t OXMTLV::pack(uint8_t *buffer) {
    uint32_t header = hton32(
        OXMTLV::make_header(this->class__, this->field_, this->has_mask_,
            this->length_));
...
uint16_t OXMTLV::oxm_class(uint32_t header) {
    return ((header) >> 16);
}

uint8_t OXMTLV::oxm_field(uint32_t header) {
    return (((header) >> 9) & 0x7f);
}

bool OXMTLV::oxm_has_mask(uint32_t header) {
    return (((header) >> 8) & 1);

}

uint8_t OXMTLV::oxm_length(uint32_t header) {
    return ((header) & 0xff);
}
...
OXMTLV * Match::oxm_field(uint8_t field) {
    return this->oxm_tlvs_[field];
}
