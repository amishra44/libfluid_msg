...
class CopyFieldAction: public Action {
private:
    uint16_t n_bits_;
    uint16_t src_offset_;
    uint16_t dst_offset_;
    std::vector<uint32_t> oxm_ids_;
public:
    CopyFieldAction();
    CopyFieldAction(uint16_t n_bits, uint16_t src_offset, uint16_t dst_offset, std::vector<uint32_t> oxm_ids_);
    CopyFieldAction(const CopyFieldAction &other);
    ~CopyFieldAction() {
    }
    virtual bool equals(const Action & other);
    CopyFieldAction& operator=(CopyFieldAction other);
    size_t pack(uint8_t *buffer);
    of_error unpack(uint8_t *buffer);
    virtual CopyFieldAction* clone() {
        return new CopyFieldAction(*this);
    }
    uint16_t n_bits() {
        return this->n_bits_;
    }
    void n_bits(uint16_t n_bits) {
        this->n_bits_ = n_bits;
    }
    uint16_t src_offset() {
        return this->src_offset_;
    }
    void src_offset(uint16_t src_offset) {
        this->src_offset_ = src_offset;
    }
    uint16_t dst_offset() {
        return this->dst_offset_;
    }
    void dst_offset(uint16_t dst_offset) {
        this->dst_offset_ = dst_offset;
    }
    std::vector<uint32_t> oxm_ids() {
        return this->oxm_ids_;
    }
    void oxm_ids(std::vector<uint32_t> oxm_ids);
    friend void swap(CopyFieldAction& first, CopyFieldAction& second);
};
