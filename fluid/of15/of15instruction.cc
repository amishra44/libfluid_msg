...
Instruction* Instruction::make_instruction(uint16_t type) {
    switch (type) {
        case (of15::OFPIT_GOTO_TABLE): {
            return new GoToTable();
        }
        case (of15::OFPIT_WRITE_METADATA): {
            return new WriteMetadata();
        }
        case (of15::OFPIT_CLEAR_ACTIONS): {
            return new ClearActions();
        }
        case (of15::OFPIT_WRITE_ACTIONS): {
            return new WriteActions();
        }
        case (of15::OFPIT_APPLY_ACTIONS): {
            return new ApplyActions();
        }
//        case (of15::OFPIT_METER): { //Aman
//            return new Meter();
//        }
        case (of15::OFPIT_EXPERIMENTER): {
...
size_t Instruction::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_header *in = (struct of15::ofp_instruction_header *) buffer; //Aman
...
GoToTable::GoToTable(uint8_t table_id)
    : Instruction(of15::OFPIT_GOTO_TABLE,
          sizeof(struct of15::ofp_instruction_goto_table)),
      set_order_(60) {
...
size_t GoToTable::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_goto_table *go =
        (struct of15::ofp_instruction_goto_table *) buffer;
...
WriteMetadata::WriteMetadata(uint64_t metadata, uint64_t metadata_mask)
    : Instruction(of15::OFPIT_WRITE_METADATA,
          sizeof(struct of15::ofp_instruction_write_metadata)),
      set_order_(50) {
...
size_t WriteMetadata::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_write_metadata *wm =
        (struct of15::ofp_instruction_write_metadata *) buffer;
    Instruction::pack(buffer);
    memset(wm->pad, 0x0, 4);
...
WriteActions::WriteActions(ActionSet actions_)
    : Instruction(of15::OFPIT_WRITE_ACTIONS,
          sizeof(struct of15::ofp_instruction_actions)),
      set_order_(40) {
    actions(actions_);
}
...
size_t WriteActions::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_actions *ia =
        (struct of15::ofp_instruction_actions*) buffer;
    Instruction::pack(buffer);
    memset(ia->pad, 0x0, 4);
    uint8_t *p = buffer + sizeof(struct of15::ofp_instruction_actions);
...
of_error WriteActions::unpack(uint8_t* buffer) {
    ...
    this->actions_.length(
        this->length_ - sizeof(struct of15::ofp_instruction_actions));
    this->actions_.unpack(p);
    return 0;
}

ApplyActions::ApplyActions(ActionList actions_)
    : Instruction(of15::OFPIT_APPLY_ACTIONS,
          sizeof(struct of15::ofp_instruction_actions)),
      set_order_(20) {
    actions(actions_);
}
...
size_t ClearActions::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_header *ia = (struct of15::ofp_instruction_header*) buffer; //Aman
    Instruction::pack(buffer);
    memset(ia->pad, 0x0, 4);
    return 0;
}
...
//    : Instruction(of15::OFPIT_METER,
//          sizeof(struct of15::ofp_instruction_meter)),
//      set_order_(10) {
...
//    struct of15::ofp_instruction_meter *im =
//        (struct of15::ofp_instruction_meter *) buffer;
//    Instruction::pack(buffer);
...
InstructionExperimenter::InstructionExperimenter(uint32_t experimenter)
    : Instruction(of15::OFPIT_EXPERIMENTER,
        sizeof(struct of15::ofp_instruction_experimenter_header)) { //Aman
...
size_t InstructionExperimenter::pack(uint8_t* buffer) {
    struct of15::ofp_instruction_experimenter_header *ie = //Aman
        (struct of15::ofp_instruction_experimenter_header *) buffer; //Aman
    Instruction::pack(buffer);
