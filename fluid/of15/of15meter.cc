...
MeterBand::MeterBand()
    : type_(0),
      rate_(0),
      burst_size_(0),
      len_(sizeof(struct of15::ofp_meter_band_header)) {
...
size_t MeterBand::pack(uint8_t* buffer) {
    struct of15::ofp_meter_band_header *bh =
        (struct of15::ofp_meter_band_header *) buffer;
...
MeterBand * MeterBand::make_meter_band(uint16_t type) {
    switch (type) {
        case (of15::OFPMBT_DROP): {
            return new MeterBandDrop();
        }
        case (of15::OFPMBT_DSCP_REMARK): {
            return new MeterBandDSCPRemark();
        }
        case (of15::OFPMBT_EXPERIMENTER): {
...
size_t MeterBandList::pack(uint8_t* buffer) {
    uint8_t *p = buffer;
    for (std::list<MeterBand*>::iterator it = this->band_list_.begin(), end =
        this->band_list_.end(); it != end; it++) {
        (*it)->pack(p);
        p += (*it)->len();
    }
    return 0;
...
MeterBandDrop::MeterBandDrop()
    : MeterBand(of15::OFPMBT_DROP, 0, 0) {
    this->len_ = sizeof(struct of15::ofp_meter_band_drop);
}

MeterBandDrop::MeterBandDrop(uint32_t rate, uint32_t burst_size)
    : MeterBand(of15::OFPMBT_DROP, rate, burst_size) {
    this->len_ = sizeof(struct of15::ofp_meter_band_drop);
}

size_t MeterBandDrop::pack(uint8_t* buffer) {
    struct of15::ofp_meter_band_drop *bd =
        (struct of15::ofp_meter_band_drop*) buffer;
...
MeterBandDSCPRemark::MeterBandDSCPRemark()
    : MeterBand(of15::OFPMBT_DSCP_REMARK, 0, 0),
      prec_level_(0) {
    this->len_ = sizeof(struct of15::ofp_meter_band_dscp_remark);
}

MeterBandDSCPRemark::MeterBandDSCPRemark(uint32_t rate, uint32_t burst_size,
    uint8_t prec_level)
    : MeterBand(of15::OFPMBT_DSCP_REMARK, rate, burst_size) {
...
size_t MeterBandDSCPRemark::pack(uint8_t* buffer) {
    struct of15::ofp_meter_band_dscp_remark *bd =
        (struct of15::ofp_meter_band_dscp_remark*) buffer;
    MeterBand::pack(buffer);
...
MeterBandExperimenter::MeterBandExperimenter()
    : experimenter_(0) {
    this->len_ = sizeof(struct of15::ofp_meter_band_experimenter);
}

MeterBandExperimenter::MeterBandExperimenter(uint32_t rate, uint32_t burst_size,
    uint32_t experimenter)
    : MeterBand(of15::OFPMBT_EXPERIMENTER, rate, burst_size) {
...
size_t MeterBandExperimenter::pack(uint8_t* buffer) {
    struct of15::ofp_meter_band_experimenter *be =
        (struct of15::ofp_meter_band_experimenter*) buffer;
    MeterBand::pack(buffer);
...
MeterConfig::MeterConfig()
    : flags_(0),
      meter_id_(0),
      length_(sizeof(struct of15::ofp_meter_desc)) { //Aman
}
...
MeterConfig::MeterConfig(uint16_t flags, uint32_t meter_id, MeterBandList bands)
    ...
    this->length_ = sizeof(struct of15::ofp_meter_desc) + bands.length(); //Aman
}
...
size_t MeterConfig::pack(uint8_t* buffer) {
    struct of15::ofp_meter_desc *mc = (struct of15::ofp_meter_desc*) buffer; //Aman
    mc->length = hton16(this->length_);
    mc->flags = hton16(this->flags_);
    mc->meter_id = hton32(this->meter_id_);
    uint8_t *p = buffer + sizeof(struct of15::ofp_meter_desc); //Aman
    this->bands_.pack(p);
    return this->length_;
}
...
of_error MeterConfig::unpack(uint8_t* buffer) {
    ...
    this->bands_.length(this->length_ - sizeof(struct of15::ofp_meter_desc)); //Aman
    uint8_t *p = buffer + sizeof(struct of15::ofp_meter_desc); //Aman
...
size_t MeterFeatures::pack(uint8_t* buffer) {
    struct of15::ofp_meter_features *mf =
        (struct of15::ofp_meter_features *) buffer;
...
size_t BandStats::pack(uint8_t* buffer) {
    struct of15::ofp_meter_band_stats *mb =
        (struct of15::ofp_meter_band_stats *) buffer;
...
MeterStats::MeterStats()
    : meter_id_(0),
      ...
      duration_sec_(0),
      duration_nsec_(0),
      len_(sizeof(struct of15::ofp_meter_stats)) {

}

MeterStats::MeterStats(uint32_t meter_id, uint32_t flow_count,
    uint64_t packet_in_count, uint64_t byte_in_count, uint32_t duration_sec,
...
MeterStats::MeterStats(uint32_t meter_id, uint32_t flow_count,
    ...
    this->len_ = sizeof(struct of15::ofp_meter_stats)
        + band_stats.size() * sizeof(struct of15::ofp_meter_band_stats);
}
...
size_t MeterStats::pack(uint8_t* buffer) {
    struct of15::ofp_meter_stats *ms = (struct of15::ofp_meter_stats*) buffer;
    ms->meter_id = hton32(this->meter_id_);
    ms->len = hton16(this->len_);
//    ms->flow_count = hton32(this->flow_count_); //Aman
    ...
    uint8_t *p = buffer + sizeof(struct of15::ofp_meter_stats);
    for (std::vector<BandStats>::iterator it = this->band_stats_.begin();
        it != this->band_stats_.end(); ++it) {
        it->pack(p);
        p += sizeof(struct of15::ofp_meter_band_stats);
    }
    return 0;
}
...
//    this->flow_count_ = hton32(ms->flow_count); //Aman
    ...
    uint8_t *p = buffer + sizeof(struct of15::ofp_meter_stats);
    size_t len = this->len_ - sizeof(struct of15::ofp_meter_stats);
    while (len) {
        BandStats stats;
        stats.unpack(p);
        this->band_stats_.push_back(stats);
        p += sizeof(struct of15::ofp_meter_band_stats);
        len -= sizeof(struct of15::ofp_meter_band_stats);
    }
    return 0;
}
...
void MeterStats::band_stats(std::vector<BandStats> band_stats) {
    ...
    this->len_ += band_stats.size() * sizeof(struct of15::ofp_meter_band_stats);
}

void MeterStats::add_band_stats(BandStats stats) {
    this->band_stats_.push_back(stats);
    this->len_ += sizeof(struct of15::ofp_meter_band_stats);
}
