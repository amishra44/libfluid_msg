...
public:
    ...
    virtual SetVLANVIDAction* clone() {
        return new SetVLANVIDAction(*this);
    }
    virtual bool equals(const Action & other);
    size_t pack(uint8_t *buffer);
    of_error unpack(uint8_t *buffer);
...
public:
    ...
    virtual SetVLANPCPAction* clone() {
        return new SetVLANPCPAction(*this);
    }
    virtual bool equals(const Action & other);
    size_t pack(uint8_t *buffer);
    of_error unpack(uint8_t *buffer);
    ...
    void vlan_pcp(uint8_t vlan_pcp) {
...
public:
    ...
    EthAddress dl_addr() {
        return this->dl_addr_;
    }
    void dl_addr(const EthAddress &dl_addr) {
        this->dl_addr_ = dl_addr;
    }
};
...
public:
    ...
    IPAddress nw_addr() {
        return this->nw_addr_;
    }
    void nw_addr(const IPAddress &nw_addr) {
        this->nw_addr_ = nw_addr;
    }
};
